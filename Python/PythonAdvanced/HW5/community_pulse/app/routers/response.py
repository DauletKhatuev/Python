...
from HW5.community_pulse.app.models import db, Question, Statistic, Response
...
def add_response():
    ...
    if not statistic:
        statistic = Statistic(question_id=question.id, agree_count=0,
                              disagree_count=0)
    ...
    db.session.commit()
    return jsonify({'message': f"Ответ на вопрос {question.id} добавлен"}),
    201